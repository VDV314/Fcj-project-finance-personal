...
Trong phần này, bạn sẽ tạo một hàm AWS Lambda để xử lý dữ liệu chi tiêu và ngân sách từ ứng dụng Finance Tracker.

## Tổng quan

Hàm Lambda sẽ:
- Nhận yêu cầu từ API Gateway
- Xác thực và xử lý dữ liệu chi tiêu/ngân sách
- Thực hiện các thao tác đọc/ghi trên bảng DynamoDB
- Trả về các phản hồi được định dạng cho frontend
...
![Create Function](/images/5-Workshop/5.4-Lambda/CreateFunction.png)

### 2. Cấu hình Thông tin Cơ bản

1. Chọn **Author from scratch**
2. Cấu hình như sau:
   - **Function name**: `ExpenseTrackerFunction`
   - **Runtime**: `Python 3.14`
   - **Architecture**: `x86_64`

![Basic Information](/images/5-Workshop/5.4-Lambda/Createfunction1.png)

### 3. Cấu hình Execution Role

1. Mở rộng **Change default execution role**
2. Chọn **Create a new role with basic Lambda permissions**

![Execution Role](/images/5-Workshop/5.4-Lambda/execution-role.png)

### 4. Cấu hình Function URL (Tùy chọn)

Bật quyền truy cập HTTP trực tiếp cho hàm của bạn:
1. Cuộn xuống **Additional configurations**
2. Trong phần **Networking**, đánh dấu **Enable Function URL**
3. Chọn **Auth type**: `NONE` (cho workshop này)

![Function URL](/images/5-Workshop/5.4-Lambda/function-url.png)

4. Nhấp **Create Function**

### 5. Thêm Mã Lambda Function

1. Trong phần **Code source**, bạn sẽ thấy trình soạn thảo mã nội tuyến
2. Thay thế mã mặc định bằng mã Python sau:
